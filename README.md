...
This module is **built natively for Hyvä** and fully tested on Hyvä 1.x.

| Feature | Hyvä | Luma / Blank |
|---|---|---|
| Promo bar | ✅ | ✅ |
| Product badge (PDP) | ✅ | ✅ |
| Product badge (CLP) | ✅ | ✅ |
...
**No jQuery. No RequireJS on the frontend.** All frontend components use vanilla JavaScript only — no Alpine.js dependency either, so the module works regardless of whether Alpine.js is available. The admin preview uses Magento's built-in RequireJS as expected in the admin panel.

The product badge uses DOM injection with `MutationObserver` to handle Hyvä's Alpine.js rendered product cards, infinite scroll, and filter updates correctly.
